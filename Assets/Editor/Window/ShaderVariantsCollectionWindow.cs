...
namespace GameEditor.Window
{
    public class ShaderVariantsCollectionWindow : EditorWindow
    {
        ...
        {
            var window =
                GetWindow(typeof(ShaderVariantsCollectionWindow), false, "变体收集工具", true) as
                    ShaderVariantsCollectionWindow;
        ...
        void OnGUI()
        {
            ...
            if (GUILayout.Button(
                    EditorGUIUtility.TrTextContent("Print Shader Usage To File", "打印当前路径下材质球的Shader使用情况到csv文件")))
            {
        ...
        void Collection()
        {
            ...
            log =
                $"Found  {collection.shaderCount} Shaders & {collection.variantCount} Variants  Used In All Materials.";
        }
        ...
        void AddVariantOfPassTypeToCollection(PassType passType, Material material)
        {
            ...
            if (shaderAllkeyworlds.Contains("FOG_LINEAR") || shaderAllkeyworlds.Contains("FOG_EXP") ||
                shaderAllkeyworlds.Contains("FOG_EXP2"))
            {
        ...
        void Save()
        {
            ...
            if (collection && !string.IsNullOrEmpty(savePath))
            {
                ...
                log =
                    $"(shader:{collection.shaderCount}, variant:{collection.variantCount}) Collection Saved At: {savePath} !";
    ...
    public class ShaderUtilImpl
    {
        ...
        public static string[] GetShaderGlobalKeywords(Shader shader)
        {
            if (GetShaderGlobalKeywords_impl == null)
                GetShaderGlobalKeywords_impl = Delegate.CreateDelegate
                (
                    typeof(GetShaderGlobalKeywords_type),
                    typeof(UnityEditor.ShaderUtil).GetMethod("GetShaderGlobalKeywords",
        ...
        public static string[] GetAllGlobalKeywords()
        {
            if (GetAllGlobalKeywords_impl == null)
                GetAllGlobalKeywords_impl = Delegate.CreateDelegate
                (
                    typeof(GetAllGlobalKeywords_type),
                    typeof(UnityEditor.ShaderUtil).GetMethod("GetAllGlobalKeywords",
                        BindingFlags.Static | BindingFlags.NonPublic)
        ...
        public static string[] GetShaderLocalKeywords(Shader shader)
        {
            if (GetShaderLocalKeywords_impl == null)
                GetShaderLocalKeywords_impl = Delegate.CreateDelegate
                (
                    typeof(GetShaderLocalKeywords_type),
                    typeof(UnityEditor.ShaderUtil).GetMethod("GetShaderLocalKeywords",
                        BindingFlags.Static | BindingFlags.NonPublic)
            ...
            out string[] remainingKeywords)
        {
            if (GetShaderVariantEntriesFiltered_impl == null)
                GetShaderVariantEntriesFiltered_impl = Delegate.CreateDelegate
                (
                    typeof(GetShaderVariantEntriesFiltered_type),
                    typeof(UnityEditor.ShaderUtil).GetMethod("GetShaderVariantEntriesFiltered",
                        BindingFlags.Static | BindingFlags.NonPublic)
                ) as GetShaderVariantEntriesFiltered_type;
            GetShaderVariantEntriesFiltered_impl(shader, maxEntries, filterKeywords, excludeCollection, out passTypes,
                out keywordLists, out remainingKeywords);
        }
        ...
        public static ShaderVariantEntriesData GetShaderVariantEntriesFilteredInternal(
            ...
            ShaderVariantCollection excludeCollection)
        {
            if (GetShaderVariantEntriesFilteredInternal_impl == null)
                GetShaderVariantEntriesFilteredInternal_impl = Delegate.CreateDelegate
                (
                    typeof(GetShaderVariantEntriesFilteredInternal_type),
                    typeof(UnityEditor.ShaderUtil).GetMethod("GetShaderVariantEntriesFilteredInternal",
                        BindingFlags.Static | BindingFlags.NonPublic)
