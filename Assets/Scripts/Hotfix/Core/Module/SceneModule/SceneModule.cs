...
{
    public class SceneModule : ISceneModule, IModuleAwake, IModuleDestroy
    {
        private SceneType _curSceneType;
        public SceneType CurSceneType => _curSceneType;
        ...
        public void Awake(object parameter)
        {
            ...
            SetSceneName(SceneManager.GetSceneByBuildIndex(0).name);
        }

        private void SetSceneName(string name) {
            _currentMainSceneName = name;
        }

        public void SetCurSceneType(SceneType sceneType)
        {
            _curSceneType = sceneType;
            AudioUtil.PlayBgm(_curSceneType);
        }
        ...
        public Scene LoadScene(string location, LoadSceneMode sceneMode = LoadSceneMode.Single, bool suspendLoad = false, uint priority = 100,
            Action<Scene> callBack = null, bool gcCollect = true, Action<float> progressCallBack = null)
        {
            ...
            if (sceneMode == LoadSceneMode.Additive)
            {
                ...
                {
                    Logger.Warning($"Could not load subScene while already loaded. Scene: {location}");
                    return default;
                }
            ...
            else
            {
                ...
                {
                    Logger.Warning($"Could not load MainScene while loading. CurrentMainScene: {_currentMainSceneName}.");
                    return default;
                }

                SetSceneName(location);
                ...
                G.ResourceModule.UnloadAllUnusedAssets(gcCollect).Forget();
        ...
        public bool ActivateScene(string location)
        {
            ...
            Logger.Warning($"IsMainScene invalid location:{location}");
            return false;
        }
        ...
        public void UnloadAsync(string location, Action callBack = null, Action<float> progressCallBack = null)
        {
            ...
            if (subScene != null)
            {
                ...
                {
                    Logger.Error($"Could not unload Scene while not loaded. Scene: {location}");
                    return;
                }
            ...
            Logger.Warning($"UnloadAsync invalid location:{location}");
        }
