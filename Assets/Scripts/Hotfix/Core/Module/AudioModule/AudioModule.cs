...
{
    public class AudioModule : IAudioModule, IModuleAwake, IModuleUpdate
    {
            ...
            AudioMixer audioMixer = null)
        {
            ...
            {
                throw new Exception("AudioGroupConfig[] is invalid.");
            }
            ...
            if (_audioMixer == null)
            {
                _audioMixer = GameSettings.Instance.AudioSetting.mixer;
                if(_audioMixer == null)
                    Logger.Error("AudioMixer is null,please set AudioMixer to AudioSetting.mixer.");
            }
            ...
            for (int index = 0; index < (int)AudioType.Max; ++index)
            {
                ...
                _audioCategories[index] =
                    new AudioCategory(audioGroupConfig.AgentHelperCount, _audioMixer, audioGroupConfig);
        ...
        public AudioAgent Play(AudioType type, string path, bool bLoop = false, float volume = 1.0f,
            bool bInPool = false,Action<string> complete = null)
        {
            if (_bUnityAudioDisabled)
            {
                return null;
            ...
            AudioAgent audioAgent = _audioCategories[(int)type].Play(path, bInPool,complete);
            {
                if (audioAgent != null)
                {
        ...
        public void PutInAudioPool(List<string> list)
        {
            ...
            foreach (string path in list)
            {
                ...
                {
                    AssetHandle assetData = G.ResourceModule.LoadAssetAsyncHandle<AudioClip>(path);
        ...
        /// 将部分AudioClip从对象池移出。
        /// </summary>
        public void RemoveClipFromPool(string path)
        {
