    ...
    public class AudioCategory
    {
        ...
        public AudioCategory(int maxChannel, AudioMixer audioMixer, AudioGroupConfig audioGroupConfig)
        {
            ...
            AudioMixerGroup[] audioMixerGroups =
                audioMixer.FindMatchingGroups(string.Format("Master/{0}", audioGroupConfig.AudioType.ToString()));
            ...
            InstanceRoot = new GameObject(string.Format("Audio Category - {0}", _audioMixerGroup.name)).transform;
            InstanceRoot.SetParent(G.AudioModule.InstanceRoot);
        ...
        /// <returns></returns>
        public AudioAgent Play(string path, bool bInPool = false,Action<string> complete = null)
        {
            if (!_bEnable)
            {
                return null;
            ...
            if (freeChannel >= 0)
            {
                ...
                {
                    AudioAgents[freeChannel] = AudioAgent.Create(path, this, bInPool,complete);
                }
                else
                {
                    AudioAgents[freeChannel].Load(path, bInPool, complete);
                }
            ...
            {
                Logger.Error($"Here is no channel to play audio {path}");
                return null;
            }
        }
