...
namespace Hotfix.Logic.GamePlay.Debugger
{
    public sealed class EcsSystemsDebugSystem : IEcsPreInitSystem, IEcsPostDestroySystem
    {
        ...
        public void PreInit(IEcsSystems systems)
        {
            ...
            {
                var sysType = DebuggerEx.GetCleanGenericTypeName(sys.GetType());
                if (sys is IEcsPreInitSystem)
                {
                    preInitList.Add(sysType);
                }

                if (sys is IEcsInitSystem)
                {
                    initList.Add(sysType);
                }
                ...
                if (sys is IEcsPostRunSystem)
                {
                    postRunList.Add(sysType);
                }

                if (sys is IEcsDestroySystem)
                {
                    destroyList.Add(sysType);
                }

                if (sys is IEcsPostDestroySystem)
                {
                    postDestroyList.Add(sysType);
                }
            }
