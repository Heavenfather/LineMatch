...
{
    public sealed class EcsWorldDebugSystem : IEcsPreInitSystem, IEcsRunSystem
    {
        ...
        public void Run(IEcsSystems systems)
        {
            foreach (var pair in _dirtyEntities)
            {
                ...
                if (_world.GetEntityGen(entity) > 0)
                {
                    ...
                    {
                        entityName = $"{entityName}:{DebuggerEx.GetCleanGenericTypeName(_typesCache[i])}";
                    }
                }
