...
namespace HotfixLogic
{
    ...
    public partial class MatchResultLose : UIWindow
    {
        ...
        private void OnClickContinueBtn()
        {
            ...
            G.GameItemModule.UseItem("coin", useCoin, success =>
            {
                if (success)
                {
                    ...
                    EventDispatcher.DispatchEvent(GameEventDefine.OnMatchAddStepUseCoin,
                        EventOneParam<int>.Create(useCoin));
                }
            }, false);
        }
        ...
        private void AddElements(bool isRestart = false)
        {
            ...
            if (itemList.Count > 0)
            {
                G.GameItemModule.UseItem(itemList, (cbList) =>
                {
                    ...
                    {
                        var param = EventThreeParam<List<int>, List<int>, bool>.Create(rewardIDList, new List<int>(),
                            true);
        ...
        {
            _animator.PlayUIAnimation("ani_ef_wy_00008_end", (succ) => { CloseSelf(); });
        }
