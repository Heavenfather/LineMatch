...
#pragma warning restore 0649
		...
		private void OnClickMoveBtn() {
			...
			float canvasWidth = canvasRawSize.x / 2;
			
			var posX = transform.position.x;
			if (_isShow) {
				posX = canvasWidth - halfSize.x;
			} else {
				posX = canvasWidth + halfSize.x;
			}

			MoveBtn.transform.localScale = new Vector3(_isShow ? -1 : 1, 1, 1);
			transform.DOLocalMoveX(posX, 0.2f);

		}

		public void SetGMCallback(Action callback)
		{
			GMCallback = callback;
		}
